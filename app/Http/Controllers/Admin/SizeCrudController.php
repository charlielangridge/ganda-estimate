...
class SizeCrudController extends CrudController
{
    ...
    public function sortDimensions($request)
    {
        if ($request->dim_y > $request->dim_X)
        {
            $request->dim_x ^= $request->dim_y ^= $request->dim_x ^= $request->dim_y;
        }

        $request['dim_x'] = $request->dim_x;
        $request['dim_y'] = $request->dim_y;
        return $request;
    }
}
